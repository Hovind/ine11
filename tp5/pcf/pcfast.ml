...
let rec print oc e = match e with
| EInt n ->
    fprintf oc "%d" n
| EBool b ->
    fprintf oc "%s" (if b then "true" else "false")
| EIdent s ->
    fprintf oc "%s" s
| EString s ->
    fprintf oc "\"%s\"" s
| EApp(e1, e2) ->
    fprintf oc "(%a %a)" print e1 print e2
| ELet(f, e1, e2) ->
...
| ELetrec(f, x, e1, e2) ->
    let (params, e) = params_body e1 in
    fprintf oc "(let rec %s %s %a= %a in %a)"
      f x
      (fun oc -> List.iter (fun s -> fprintf oc "%s " s)) params
      print e
      print e2
| EFun (x, e) ->
    fprintf oc "(fun %s -> %a)"  x print e
| EIf (test, e1, e2) ->
    fprintf oc "(if %a then %a else %a)" print test print e1 print e2
| EBinop(op,e1,e2) ->
    fprintf oc "(%a %s %a)" print e1 op print e2
| EMonop(op,e) ->
    fprintf oc "%s%a" op print e
;;
